...
public class ApiDriver {
        ...
        try {
            switch (mode) {
                case UPDATE:
                    ...
                    this.body = updateJSONBody(jsonBody, key, updatedValue).toString();
                    break;
                case DELETE:
                    this.body = deleteFieldFromJSONBody(jsonBody, key).toString();
                    break;
                default:
     ...
     * @return The updated JSON body as a JSONObject.
     * @throws JSONException if there is an issue with JSON processing.
     */
    public JSONObject updateJSONBody(String jsonBody, String key, Object updatedValue) throws JSONException {
        JSONObject jsonObj = new JSONObject(jsonBody);
        if (key.contains(".")) {
            String[] newKeys = key.split("\\.");
            if (newKeys.length > 2) {
                if (newKeys[0].contains("[")) {
                    JSONArray jarray = jsonObj.getJSONArray(newKeys[0].split("\\[")[0]);
                    int index = Integer.parseInt(newKeys[0].substring(newKeys[0].indexOf('[') + 1, newKeys[0].indexOf(']')));
                    jarray.put(index, updateJSONBody(jarray.get(index).toString(), key.replace(newKeys[0] + ".", ""), updatedValue));
                    jsonObj.put(newKeys[0].split("\\[")[0], jarray);
                } else {
                    jsonObj.put(newKeys[0], updateJSONBody(jsonObj.getJSONObject(newKeys[0]).toString(), key.replace(newKeys[0] + ".", ""), updatedValue));
                }
            } else {
                if (newKeys[0].contains("[")) {
                    int index = Integer.parseInt(newKeys[0].substring(newKeys[0].indexOf('[') + 1, newKeys[0].indexOf(']')));
                    if (newKeys[1].contains("[")) {
                        JSONArray jarray = jsonObj.getJSONArray(newKeys[0].split("\\[")[0]);
                        jarray.put(index, arrayUpdate((JSONObject) jarray.get(index), newKeys[1], updatedValue));
                        jsonObj.put(newKeys[0].split("\\[")[0], jarray);
                    } else {
                        jsonObj.getJSONArray(newKeys[0].split("\\[")[0]).getJSONObject(index).put(newKeys[1], updatedValue);
                    }
                } else {
                    if (newKeys[1].contains("[")) {
    ...
    public JSONObject deleteFieldFromJSONBody(String jsonBody, String key) throws JSONException {
        JSONObject jsonObj = new JSONObject(jsonBody);
        if (key.contains(".")) {
            String[] newKeys = key.split("\\.");
            if (newKeys.length > 2) {
                if (newKeys[0].contains("[")) {
                    JSONArray jarray = jsonObj.getJSONArray(newKeys[0].split("\\[")[0]);
                    int index = Integer.parseInt(newKeys[0].substring(newKeys[0].indexOf('[') + 1, newKeys[0].indexOf(']')));
                    jarray.put(index, deleteFieldFromJSONBody(jarray.get(index).toString(), key.replace(newKeys[0] + ".", "")));
                    jsonObj.put(newKeys[0].split("\\[")[0], jarray);
                } else {
                    jsonObj.put(newKeys[0], deleteFieldFromJSONBody(jsonObj.getJSONObject(newKeys[0]).toString(), key.replace(newKeys[0] + ".", "")));
                }
            } else {
                if (newKeys[0].contains("[")) {
                    int index = Integer.parseInt(newKeys[0].substring(newKeys[0].indexOf('[') + 1, newKeys[0].indexOf(']')));
                    if (newKeys[1].contains("[")) {
                        JSONArray jarray = jsonObj.getJSONArray(newKeys[0].split("\\[")[0]);
                        jarray.put(index, arrayDelete((JSONObject) jarray.get(index), newKeys[1]));
                        jsonObj.put(newKeys[0].split("\\[")[0], jarray);
                    } else {
                        jsonObj.getJSONArray(newKeys[0].split("\\[")[0]).getJSONObject(index).remove(newKeys[1]);
                    }
                } else {
                    if (newKeys[1].contains("[")) {
    ...
    public JSONObject arrayDelete(JSONObject jsonObj, String key) throws JSONException {
        ...
        JSONArray array = nestObject.getJSONArray((key.split("\\["))[0]);
